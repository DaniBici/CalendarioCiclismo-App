-- ─────────────────────────────────────────────────────────────────
--  135_push_auto_dispatch_category_ambiguity_fix.sql
--
--  Fix — «column reference "category" is ambiguous» en pg_cron.
--  ────────────────────────────────────────────────────────────
--  Desde la 054, `auto_dispatch_premium_pushes()` declara `category`
--  como parámetro OUT (RETURNS TABLE(... category TEXT ...)). En las
--  sentencias UPDATE/DELETE sobre `push_auto_dispatch` el `WHERE`
--  filtra por `category = '...'` SIN cualificar, y ahí Postgres no
--  puede decidir entre la variable PL/pgSQL y la columna de la tabla:
--
--    ERROR: column reference "category" is ambiguous
--    QUERY: UPDATE push_auto_dispatch SET ... WHERE "eventKey" = ...
--           AND category = 'tv_start'
--
--  Como el cron `auto-dispatch-premium-pushes` corre cada 10 min
--  (*/10), en plena temporada esto producía 6 errores de Postgres por
--  hora de forma continua: cada tick entra en la rama `rescheduled`
--  de tv_start (los horarios de emisión se reajustan) y aborta.
--
--  Las lecturas `SELECT ... INTO` ya cualificaban con `pad.category`,
--  por eso no fallaban. Aquí se cualifican las 4 escrituras restantes
--  con `push_auto_dispatch.category`. No cambia ninguna lógica ni el
--  esquema — es un CREATE OR REPLACE idéntico a la 054 salvo esas
--  cuatro cualificaciones.
-- ─────────────────────────────────────────────────────────────────
    ...
    FOREACH v_lang IN ARRAY ARRAY['es', 'en'] LOOP
      ...
      ELSIF v_existing_src IS DISTINCT FROM rec.source_time THEN
        ...
        UPDATE push_auto_dispatch SET "sourceTimeUtc" = rec.source_time, "updatedAt" = v_now
          WHERE "eventKey" = v_event_key AND push_auto_dispatch.category = 'race_start';
    ...
    FOREACH v_lang IN ARRAY ARRAY['es', 'en'] LOOP
      ...
      ELSIF v_existing_src IS DISTINCT FROM rec.source_time THEN
        UPDATE scheduled_push_notifications
          ...
          WHERE id = v_existing_id AND status = 'pending';
        UPDATE push_auto_dispatch SET "sourceTimeUtc" = rec.source_time, "updatedAt" = v_now
          WHERE "eventKey" = v_event_key AND push_auto_dispatch.category = 'results';
      ...
      FOREACH v_lang IN ARRAY ARRAY['es', 'en'] LOOP
        ...
        IF v_existing_id IS NOT NULL THEN
          ...
          DELETE FROM push_auto_dispatch
            WHERE "eventKey" = v_event_key AND push_auto_dispatch.category = 'tv_start';
...
COMMENT ON FUNCTION public.auto_dispatch_premium_pushes(INTEGER)
  IS 'Programa notificaciones Premium en ES y EN. Por cada evento (race_start, results, tv_start×grupo fino) genera 2 inserts con targetLanguages=[''es''] y [''en'']. Localizaciones vacías o whitespace-only se tratan como NULL. Referencias a la columna category cualificadas con push_auto_dispatch.category para evitar ambigüedad con el parámetro OUT (fix 135).';
