-- ─────────────────────────────────────────────────────────────────
--  054_push_auto_dispatch_empty_string_fix.sql
--
--  Fix 1 — strings vacíos en startLocation/finishLocation
--  ─────────────────────────────────────────────────────
--  La 053 trataba '' como string válido y producía subtítulos como
--    "Salen en 30 min · Krásná Lípa →  · 12 km"
--  con ` → ` colgante por finishLocation = ''. El fix envuelve cada
--  acceso a start_loc / finish_loc / start_loc_en / finish_loc_en
--  con `NULLIF(trim(...), '')` para que cualquier cadena vacía o
--  con solo espacios se trate como NULL y el bloque de recorrido se
--  omita por completo.
--
--  Fix 2 — limpieza de filas auto_dispatch sin idioma
--  ──────────────────────────────────────────────────
--  Las migraciones 050/051 dejaron filas en `push_auto_dispatch` y
--  `scheduled_push_notifications` con eventKey antiguo (sin sufijo
--  -es/-en). Como la 053 usa un eventKey distinto, esas filas viejas
--  ya no se reaprovechan y producen DOS notificaciones por evento:
--  la antigua genérica + las dos nuevas enriquecidas.
--
--  Se cancelan todas las scheduled pendientes auto_dispatch que NO
--  tienen targetLanguages poblado (señal inequívoca de que vienen
--  del esquema antiguo) y se purga su entrada de push_auto_dispatch.
--  Las apps NO recibirán las antiguas — sí las dos nuevas que ya
--  acaban de programarse al ejecutar 053.
-- ─────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  LOOP
    ...
    -- Normaliza '' / whitespace a NULL (fix 054)
    v_start_loc     := NULLIF(btrim(rec.start_loc),     '');
    v_finish_loc    := NULLIF(btrim(rec.finish_loc),    '');
    v_start_loc_en  := NULLIF(btrim(rec.start_loc_en),  '');
    v_finish_loc_en := NULLIF(btrim(rec.finish_loc_en), '');
...
COMMENT ON FUNCTION public.auto_dispatch_premium_pushes(INTEGER)
  IS 'Programa notificaciones Premium en ES y EN. Por cada evento (race_start, results, tv_start×grupo fino) genera 2 inserts con targetLanguages=[''es''] y [''en'']. Localizaciones vacías o whitespace-only se tratan como NULL.';

-- ── Limpieza de filas auto_dispatch heredadas de la migración 051 ──
-- (eventKey sin sufijo de idioma → no las regenera la 053)
WITH ghost_rows AS (
  SELECT id
  FROM scheduled_push_notifications
  WHERE "createdBy" = 'auto_dispatch'
    AND status = 'pending'
    AND "targetLanguages" IS NULL
),
cancelled_count AS (
  UPDATE scheduled_push_notifications
...
DELETE FROM push_auto_dispatch
  WHERE "scheduledNotificationId" IN (SELECT id FROM cancelled_count);
